...
import type { PlanType, SaveAnswersDTO, SaveDraftDTO } from '~~/dto/question'
...
export const registerService = defineService({
  ...
  mutations: {
    ...
    saveAnswers: {
      ...
      cacheKey: (_data: SaveAnswersDTO, variables) => [['QUESTIONS_GET_PROJECT_PROGRESS'], ['QUESTIONS_GET_PROJECT_ANSWERS', variables.projectId], ['QUESTIONS_GET_PROJECT_PROGRESS', variables.projectId]]
    },
    generateAIPrompt: {
      request: (projectId: string) => trpc().questions.generateAIPrompt.mutate({ projectId }),
      cacheKey: (_data: SaveAnswersDTO, projectId) => [['QUESTIONS_GET_PROJECT_PROGRESS', projectId]]
    }
  }
})
